...
class SparkTrainerTest extends Specification {
  ...
  /*
    "SparkTrainer(Remote)" should {
      val layer = new Rank3TensorLayer((2, 1) → 4, Sigmoid)
      val net = new BasicNetwork(Seq(layer))
      val conf = new SparkConf().setMaster("spark://irdata.kaist.ac.kr:7077").setAppName("SparkTrainer Test")
      val sc = new SparkContext(conf)
      val train = new SparkTrainer(net = net,
        algorithm = new StochasticGradientDescent(rate = 0.8, l2decay = 0.0001),
        sc = sc,
        param = DistBeliefCriteria(miniBatch = 16, fetchStep = 5, updateStep = 2, numCores = 4),
        stops = StoppingCriteria(maxIter = 100000))
